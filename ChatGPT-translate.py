...
def translate(key, target_language, text, use_azure=False, api_base="", deployment_name="", options=None):
    ...
    messages = [{
        ...
        "user",
        "content":
        f"You are a text localization assistant. Please Localize the following text into {target_language}. Retain the original format. Return only the translation and nothing else:\n{text}",
    }]
    if use_azure:
